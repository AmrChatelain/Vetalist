...
export async function getClientDashboardData() {
  ...
  const [appointments, pets] = await Promise.all([
    db.appointment.findMany({
      ...
      include: {
        vet: {
          select: {
            id: true, clinicName: true, city: true,
            street: true, zipCode: true, photoUrl: true,
  ...
  const past = appointments
    .filter((a) => a.startTime < now || a.status === "DONE" || a.status === "CANCELLED")
  ...
  try {
    ...
    const apt = await db.appointment.findUnique({
      ...
      include: {
        vet: {
          select: {
            clinicName: true, street: true, zipCode: true, city: true,
            user: { select: { firstName: true, lastName: true } },
          },
        },
    ...
    if (apt.status !== "PENDING" && apt.status !== "CONFIRMED") {
      return { success: false, error: "Ce rendez-vous ne peut plus être annulé" }
    }
    ...
    try {
      ...
      await db.emailLog.create({
        data: { appointmentId, recipientEmail: client.email, emailType: "CANCELLATION" },
      })
    } catch (emailErr) {
...
export async function changePassword(data: {
  ...
  try {
    ...
    if (!data.currentPassword || !data.newPassword || data.newPassword.length < 8) {
      return { success: false, error: "8 caractères minimum pour le nouveau mot de passe" }
    }
    ...
    if (!user?.passwordHash) {
      return { success: false, error: "Compte Google — aucun mot de passe à modifier" }
    }
    ...
    await db.user.update({ where: { id: client.id }, data: { passwordHash: hash } })
