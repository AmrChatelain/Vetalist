...
const AVATAR_IDS = [
  { id: 12, src: "https://res.cloudinary.com/dhvha4h1o/image/upload/v1777289839/1670776925850_i0nhgt.jpg" },
  { id: 10, src: "https://res.cloudinary.com/dhvha4h1o/image/upload/v1777289510/2026_04_19_21_57_IMG_7620_xaaxmn.jpg" },
  { id: 11, src: null },
]
...
export function Hero({
  title    = "Des soins experts pour votre meilleur ami, partout en France.",
  subtitle = "Prenez rendez-vous avec un vétérinaire de confiance en quelques secondes. Sans attente, sans stress — juste des animaux heureux et en bonne santé.",
}: HeroProps) {
  ...
  return (
    <section className="relative min-h-[90vh] flex items-center overflow-hidden bg-[#fdf6f0]">
      ...
      <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
        ...
        <span className="absolute top-[15%] left-[10%] text-6xl opacity-[0.04] select-none" aria-hidden="true">🐾</span>
        <span className="absolute bottom-[20%] right-[15%] text-8xl opacity-[0.04] select-none" aria-hidden="true">🐾</span>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full relative z-10 py-28 lg:py-36">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="flex flex-col space-y-8 text-center lg:text-left">
            ...
            <div className="inline-flex items-center gap-3 px-4 py-2.5 rounded-full bg-white/70 backdrop-blur-sm border border-amber-100 w-fit mx-auto lg:mx-0 shadow-sm">
              <div className="flex -space-x-2" aria-hidden="true">
                {AVATAR_IDS.map(({ id, src }) => (
                  <div key={id} className="w-7 h-7 rounded-full border-2 border-white bg-slate-200 overflow-hidden relative">
                    <Image
            ...
            {/* ✅ SearchBar with autocomplete replaces the old input */}
            <div className="w-full max-w-xl mx-auto lg:mx-0">
              ...
              <div className="flex flex-wrap gap-2 mt-3 justify-center lg:justify-start">
                {["Paris", "Lyon", "Marseille", "Chirurgie", "Urgences"].map((chip) => (
                  <button
                    key={chip}
                    type="button"
                    onClick={() => handleSearch({ q: chip })}
                    className="text-xs px-3 py-1.5 bg-white/80 border border-slate-200 rounded-full text-slate-600 hover:border-violet-300 hover:text-violet-600 transition-all"
                  >
                    {chip}
                  </button>
                ))}
            ...
            <div className="flex items-center justify-center lg:justify-start gap-4 pt-1">
              <Button
                ...
                size="sm"
                asChild
                className="border-slate-200 bg-white/60 hover:bg-white transition-all rounded-full font-semibold text-slate-700 text-sm"
              >
                <Link href="/register">
                  Rejoindre en tant que vétérinaire
            ...
            {/* Trust indicators */}
            <div className="flex items-center justify-center lg:justify-start gap-8 pt-1">
              <div className="flex items-center gap-2 text-slate-500">
                <ShieldCheck className="w-5 h-5 text-emerald-500 shrink-0" />
                <span className="text-sm font-medium">Vétérinaires vérifiés</span>
              </div>
              <div className="flex items-center gap-2 text-slate-500">
                <Star className="w-5 h-5 text-amber-400 fill-amber-400 shrink-0" />
                <span className="text-sm font-medium">4.9 / 5 note moyenne</span>
              </div>
            </div>
          </div>
          ...
          <div className="relative hidden lg:block">
            <div className="relative z-10 w-full aspect-square rounded-[3rem] overflow-hidden shadow-2xl border-8 border-white">
              <Image
                ...
                alt="Vétérinaire prenant soin d'un animal"
                fill
                priority
                className="object-cover"
            ...
            <div className="absolute -bottom-6 -left-8 z-20 bg-white/90 backdrop-blur-xl p-4 rounded-2xl shadow-xl border border-white/80 animate-bounce-slow">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-400 mt-0.5">Disponible 24h/24</p>
                </div>
              </div>
            </div>

            <div className="absolute -top-10 -right-10 w-64 h-64 bg-[#fda4af]/20 rounded-full blur-3xl -z-10" aria-hidden="true" />
          </div>
        </div>
      </div>
