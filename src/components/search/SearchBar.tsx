...
export function SearchBar({
 ...
 const debounceRef = useRef<ReturnType<typeof setTimeout> | undefined>(undefined)
  ...
  function handleSelect(s: Suggestion) {
    setOpen(false)
    if (s.type === "vet" && s.id) { router.push(`/vets/${s.id}`); return }
    const p: Record<string, string> = {}
    if (s.type === "city")      p.city     = s.label
    if (s.type === "specialty") p.specialty = s.label
    if (query) p.q = query
    if (onSearch) onSearch(p)
    else router.push(buildUrl(p))
    setQuery(s.label)
  }

  function handleSubmit() {
    if (!query.trim()) {
      // Empty submit → show all vets, preserve other filters
    ...
    setOpen(false)
    const p: Record<string, string> = { q: query.trim() }
    if (onSearch) onSearch(p)
    else router.push(buildUrl(p))
  }
  ...
  function handleClear() {
    ...
    // Only navigate if we're on the search page (no onSearch = not landing page)
    if (!onSearch) {
      const sp = new URLSearchParams(params.toString())
      sp.delete("q")
      sp.delete("city")
    ...
    <div className="relative w-full">
      ...
      {open && suggestions.length > 0 && (
        ...
        >
          ...
          {cities.length > 0 && (
            <div>
              <div className="px-4 pt-3 pb-1 text-xs font-bold text-slate-400 uppercase tracking-wider">Villes</div>
              {cities.map((s) => {
                const idx = getIdx()
                return (
                  <button
                  ...
                  >
                    ...
                    <span className="ml-auto text-xs text-slate-400">Ville</span>
                  </button>
                )
              })}
            </div>
          ...
          {specialties.length > 0 && (
            <div className={cities.length > 0 ? "border-t border-slate-100" : ""}>
              <div className="px-4 pt-3 pb-1 text-xs font-bold text-slate-400 uppercase tracking-wider">Spécialités</div>
              {specialties.map((s) => {
                const idx = getIdx()
                return (
                  <button
                  ...
                  >
                    {iconMap.specialty}
                    <span className="text-sm text-slate-700">{s.label}</span>
                    <span className="ml-auto text-xs text-slate-400">Spécialité</span>
                  </button>
                )
              })}
            </div>
          ...
          {vets.length > 0 && (
            <div className={(cities.length > 0 || specialties.length > 0) ? "border-t border-slate-100" : ""}>
              <div className="px-4 pt-3 pb-1 text-xs font-bold text-slate-400 uppercase tracking-wider">Vétérinaires</div>
              {vets.map((s) => {
                const idx = getIdx()
                return (
                  <button
                  ...
                  >
                    ...
                    <span className="ml-auto text-xs text-slate-400 flex-shrink-0">Vétérinaire</span>
                  </button>
                )
              })}
            </div>
          ...
          <div className="px-4 py-2.5 border-t border-slate-100 bg-slate-50 flex items-center gap-1.5">
            ...
            <span className="text-xs text-slate-400">
              Appuyez sur{" "}
              <kbd className="bg-white border border-slate-200 rounded px-1 text-[10px]">Entrée</kbd>{" "}
              pour rechercher
            </span>
          </div>
        </div>
